...
suite("Utility Tests ", () => {
	test("should be able to get hostname", () => {
		const hostname = Utility.getHostName(
			TestConstants.IotHubConnectionString
		);
	...
	test("should be able to get postfix of hostname", () => {
		assert.equal(
			Utility.getPostfixFromHostName(TestConstants.IotHubHostName),
			"azure-devices.net"
		);
	});

	test("should be able to get config", () => {
		const iotHubD2CMessageStringify = Utility.getConfig<boolean>(
	...
	test("should be able to get auto refresh enable flag", () => {
		const autoRefreshEnable = Utility.getConfig<boolean>(
	...
	test("should be able to get auto refresh interval", () => {
		const autoRefreshIntervalInSeconds = Utility.getConfig<boolean>(
	...
	test("should be able to generate SAS Token for IoT Hub", () => {
		const sasTokenForService = Utility.generateSasTokenForService(
			TestConstants.IotHubConnectionString,
			10
		);
		assert.equal(
			/^SharedAccessSignature sr=iot-hub-test.azure-devices.net&sig=.+&skn=iothubowner&se=.+$/.test(
				sasTokenForService
			),
			true
		);
	...
	test("should be able to generate SAS Token for Device", () => {
		const sasTokenForDevice = Utility.generateSasTokenForDevice(
			TestConstants.DeviceConnectionString,
			10
		);
		assert.equal(
			/^SharedAccessSignature sr=iot-hub-test.azure-devices.net%2Fdevices%2Fdevice1&sig=.+&se=.+$/.test(
				sasTokenForDevice
			),
			true
		);
	...
	test("should be able to validate target condition", () => {
		...
		assert.equal(
			Utility.isValidTargetCondition("tags.city='Shanghai'"),
			true
		);

		assert.equal(
			Utility.isValidTargetCondition(
				"properties.reported.lastStatus='200'"
			),
			true
		);

		assert.equal(
			Utility.isValidTargetCondition("deviceId='device1'"),
			true
		);

		assert.equal(
			Utility.isValidTargetCondition(
				"properties.desired.lastStatus='200'"
			),
			false
		);
