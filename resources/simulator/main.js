...
const app = new Vue({
	...
	methods: {
		async getPersistedInputs() {
			await axios
				...
				.then(async (res) => {
					...
					this.formItem.deviceConnectionStrings =
		...
		async getInputDeviceList() {
			const list = (
				await axios.get(`${this.endpoint}/api/getinputdevicelist`)
			).data;
		...
		async generateDummyJson() {
			...
			const validated = await axios
				...
				.catch((err) => {
					this.textArea.generatedMessage =
						"Malformed dummy json syntax.";
					return false;
				});
			return validated;
		},
