...
import type { ResultWithHttpResponse } from "azure-iot-common";
import {
	ConnectionString as DeviceConnectionString,
	SharedAccessSignature as DeviceSharedAccessSignature,
...
import type { ResultWithIncomingMessage } from "azure-iothub/dist/interfaces";
...
export class Utility {
	...
	public static adjustFilePath(filePath: string): string {
		...
		if (
			...
			winshellLowercase.indexOf("git") > -1
		) {
			// Git Bash
			return filePath
		...
		return modules
			.map((module) => {
				...
				if (module.moduleId === "$edgeHub") {
					isConnected =
						isConnected ||
						(edgeAgent as any).connectionState === "Connected";
					if (isConnected) {
					...
					resolve(
						modules.map((module) => {
							if (module.authentication.symmetricKey.primaryKey) {
								(module as any).connectionString =
									Utility.createModuleConnectionString(
										hostName,
										deviceId,
										module.moduleId,
	...
	public static async updateModuleTwin(
		iotHubConnectionString: string,
		deviceId: string,
		moduleId: string,
		twin: any,
	...
	public static async getInputDevice(
		deviceItem: DeviceItem,
		eventName: string,
		onlyEdgeDevice = false,
		iotHubConnectionString?: string,
	): Promise<DeviceItem> {
		if (deviceItem) {
			if (eventName) {
	...
	public static async getDeviceList(
		...
		return deviceList.map((device) => {
			const isConnected =
				device.connectionState.toString() === "Connected";
	...
	public static createModuleConnectionString(
		hostName: string,
		deviceId: string,
		moduleId: string,
	...
	public static parseReportedSamplingMode(twin: any): boolean {
		const reportedDistributedTwinObject =
			twin.properties.reported[Constants.DISTRIBUTED_TWIN_NAME];
		if (
			reportedDistributedTwinObject.sampling_mode === undefined ||
			reportedDistributedTwinObject.sampling_mode.value === undefined
		) {
			return undefined;
		}
		return (
	...
	public static parseDesiredSamplingRate(twin: any): number {
		return twin.properties.desired[Constants.DISTRIBUTED_TWIN_NAME]
			.sampling_rate;
	}
	...
	public static generateIoTHubAxiosRequestConfig(
		iotHubConnectionString: string,
		url: string,
		method: Method,
		data?: any,
	...
	public static getReportedInterfacesFromDigitalTwin(interfaces) {
		return (
			...
			interfaces.interfaces[Constants.modelDiscoveryInterfaceName]
				.properties &&
			interfaces.interfaces[Constants.modelDiscoveryInterfaceName]
				.properties.modelInformation &&
			interfaces.interfaces[Constants.modelDiscoveryInterfaceName]
				.properties.modelInformation.reported &&
			interfaces.interfaces[Constants.modelDiscoveryInterfaceName]
				.properties.modelInformation.reported.value &&
			interfaces.interfaces[Constants.modelDiscoveryInterfaceName]
				...
				if (err) {
					...
					deviceList.forEach((device, index) => {
						...
						if (
							...
							deviceConnectionString =
								DeviceConnectionString.createWithSharedAccessKey(
									hostName,
									device.deviceId,
