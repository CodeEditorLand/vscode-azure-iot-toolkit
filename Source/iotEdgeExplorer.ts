...
export class IoTEdgeExplorer extends BaseExplorer {
	...
	private isValidCreateOptions(desiredProperties: any): boolean {
		return (
			this.isValidCreateOptionsHepler(desiredProperties.systemModules) &&
			this.isValidCreateOptionsHepler(desiredProperties.modules)
		);
	}
	...
	private isValidCreateOptionsHepler(modules: any): boolean {
		for (const moduleName in modules) {
			if (modules.hasOwnProperty(moduleName)) {
				try {
					let createOptions =
						modules[moduleName].settings.createOptions;
	...
	private async getDeploymentJson(filePath: string): Promise<string> {
		...
		try {
			...
			try {
				const isValid =
					(await this.isValidDeploymentJsonSchema(contentJson)) &&
	...
	private async deploy(
		...
		try {
			...
			TelemetryClient.sendEvent(Constants.IoTHubAIEdgeDeployDoneEvent, {
				...
				[Constants.errorProperties.detailedMessage]: detailedMessage,
				entry,
				from,
			});
		}
	...
	private async deployAtScale(
		...
		const targetCondition = await vscode.window.showInputBox({
			...
			placeHolder:
				"e.g. tags.environment='test', properties.reported.devicemodel='4000x', or deviceId='{id}'",
			ignoreFocusOut: true,
			validateInput: (value: string) => {
				if (!value) {
					return "The value should not be empty.";
