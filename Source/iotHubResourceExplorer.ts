...
export class IoTHubResourceExplorer extends BaseExplorer {
	...
	public async createIoTHub(
		...
		const sku = await vscode.window.showQuickPick(Object.keys(skuMap), {
			placeHolder: "Select pricing and scale tier for your IoT Hub...",
			ignoreFocusOut: true,
		});
		if (!sku) {
			return;
	...
	private async selectIoTHubItem(
		...
		do {
			...
			if (
				iotHubItem === undefined &&
				new Date().getTime() - start < 500
			) {
				retryCount--;
			} else {
				return iotHubItem;
	...
	private async getIoTHubConnectionString(
		credentials: any,
		subscriptionId: string,
		environment: Environment,
		iotHubDescription: IotHubModels.IotHubDescription,
	...
	private async getOrCreateResourceGroup(
		...
		if (pick) {
			if (pick instanceof ResourceGroupItem) {
				...
				const newGroup =
					await this.createResourceGroup(subscriptionItem);
				if (newGroup) {
