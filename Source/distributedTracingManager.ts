...
export class DistributedTracingManager extends BaseExplorer {
	...
	public async updateDistributedTracingSetting(
		...
		await this.updateDistributedTracingSettingForDevices(
			deviceIds,
	...
	public async updateDistributedTracingSettingForDevices(
		deviceIds: string[],
		...
		if (updateType !== DistributedSettingUpdateType.OnlySamplingRate) {
			const selectedItem: SamplingModeItem =
				await vscode.window.showQuickPick(
		...
		await vscode.window.withProgress(
			...
			async () => {
				try {
					const result = await this.updateDeviceTwin(
						mode,
						samplingRate,
						iotHubConnectionString,
						deviceIds,
					...
					TelemetryClient.sendEvent(
						Constants.IoTHubAIUpdateDistributedSettingDoneEvent,
						{
							...
							DeviceCount: deviceIds.length.toString(),
	...
	private async updateDeviceTwin(
		...
		if (enable !== undefined) {
			twinPatch.properties.desired[
				Constants.DISTRIBUTED_TWIN_NAME
			].sampling_mode = enable ? 1 : 2;
		}
	...
	private async promptForSamplingRate(
		...
		let samplingRate: string = await vscode.window.showInputBox({
			prompt,
